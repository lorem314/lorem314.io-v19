...
const Theme = () => {
  const { preferredTheme, setPreferredTheme } = useContext(GlobalContext)
  ...
  return (
    <div className="shrink-0 select-none">
      <label className="relative inline-block h-8 w-16" htmlFor="theme-toggler">
        <input
        ...
        />
        <div className="absolute top-0 right-0 bottom-0 left-0 flex items-center rounded-full bg-black">
          <MoonIcon className="size-4 basis-8" />
          <SunIcon className="size-4 basis-8" />
        </div>
        <FaCircle
          className={`absolute top-1 left-1 fill-neutral-300 transition-[fill] hover:fill-white ${isDark ? "translate-x-8" : "translate-x-0"} size-6 opacity-100 transition-transform`}
        />
      </label>
    </div>
  )
